document.addEventListener('DOMContentLoaded', () => {
    ...
    if (signUpForm) {
        signUpForm.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch('https://a86a-103-82-43-29.ngrok-free.app/signup', {
                    method: 'POST',
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch('https://a86a-103-82-43-29.ngrok-free.app/login', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
    ...
    const fetchUsers = async () => {
        try {
            const response = await fetch('https://a86a-103-82-43-29.ngrok-free.app/users');
